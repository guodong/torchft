...
def main() -> None:
    ...
    warmup_scheduler = LambdaLR(inner_optimizer, lr_lambda=lambda step: min(4e-4, step / 100))
    ...
    with DiLoCo(
            manager, m, inner_optimizer, outer_optimizer, sync_every=10,backup_device=device
        ) as diloco:
        while True:
